...
class PkpassParser(context: Context, uri: Uri?) {
    ...
    init {
        ...
        try {
            ...
            while ((zipInputStream.nextEntry.also { localFileHeader = it }) != null) {
                System.out.println(localFileHeader.fileName)
    ...
    private fun readJSONString(jsonObject: JSONObject, key: String, default: String?): String? {
        try {
    ...
    private fun parsePassJSON(jsonObject: JSONObject, locale: String?) {
        ...
        store = readJSONString(jsonObject, "logoText", null)
        if (store == null) store = readJSONString(jsonObject, "organizationName", null)
        if (store == null) throw IllegalArgumentException("pkpass contains neither logoText nor organizationName")
    ...
    private fun parsePassJSONBarcodeField(barcodeInfo: JSONObject) {
        ...
        cardId = try {
            barcodeInfo.getString("altText")
        } catch (ignored: JSONException) {
            barcodeInfo.getString("message")
        }
        ...
        for (fieldType in listOf("headerFields", "primaryFields", "secondaryFields", "auxiliaryFields", "backFields")) {
            ...
            try {
                ...
                for (i in 0 until fieldArray.length()) {
                    ...
                    content.append(parsePassJSONPassField(entry, locale)).append("\n")
                }
